...
DAETF = os.path.join(REPO, "methods", "daetf", "daetf")
KRY = os.path.join(REPO, "methods", "krylovnet", "krylovnet")
OUT = os.path.join(REPO, "methods", "krylovnet", "notebooks", "KrylovNet_Kaggle.ipynb")
...
`daetf/daetf/` and `krylovnet/krylovnet/` by
`tools/build_krylovnet_notebook.py` - edit the packages and regenerate rather
than editing these cells.
"""))
    cells.append(code(
        "import os\n"
        "for d in ('daetf/daetf', 'krylovnet/krylovnet'):\n"
        "    os.makedirs(d, exist_ok=True)\n"
        "for d in ('daetf', 'krylovnet'):\n"
        "    open(os.path.join(d, '__init__.py'), 'w').close()\n"
        "print('package dirs ready')"))

    for mod in DAETF_MODULES:
        with open(os.path.join(DAETF, f"{mod}.py"), "r", encoding="utf-8") as f:
            cells.append(code(f"%%writefile daetf/daetf/{mod}.py\n{f.read()}"))
    for mod in KRY_MODULES:
        with open(os.path.join(KRY, f"{mod}.py"), "r", encoding="utf-8") as f:
            cells.append(code(f"%%writefile krylovnet/krylovnet/{mod}.py\n{f.read()}"))

    cells.append(code("""
import sys
for m in [k for k in list(sys.modules) if k.startswith(('daetf', 'krylovnet'))]:
    del sys.modules[m]
sys.path.insert(0, os.getcwd())
import krylovnet.krylovnet as K
...
"""))
    cells.append(code("""
from daetf.daetf.engine import evaluate_dataset as shared_eval
from daetf.daetf.data import estimate_srf
from daetf.daetf.baselines import evaluate_all_baselines

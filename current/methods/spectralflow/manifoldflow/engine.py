"""Training / evaluation engine for ManifoldFlow (flow matching)."""
...
from daetf.daetf.data import FusionPatchDataset, estimate_srf
from daetf.daetf.degrade import FixedDegradation
from daetf.daetf.metrics import evaluate_arrays
...
def _make_model(cfg: Config, device: torch.device,
                srf: np.ndarray) -> ManifoldFlow:
    model = ManifoldFlow(cfg).to(device)
    ...
    for it_idx in range(cfg.iters):
        ...
        if it_idx % cfg.log_every == 0:
            ...
            print(f"[{it_idx}/{cfg.iters}] loss={loss['loss'].item():.4f} "
                  f"flow={loss['flow'].item():.4f} "
                  f"recon={loss['recon'].item():.4f} ({time.time() - t0:.0f}s)")
...
def evaluate_dataset(model, cfg, device, mode: str = "full") -> dict:
    ...
    """
    from daetf.daetf.engine import evaluate_dataset as _shared_eval
...
def tiled_inference(model: nn.Module, lr: np.ndarray, msi: np.ndarray,
                    ...
                    tile: int = 256, batch: int = 8) -> np.ndarray:
    h, w = lr.shape[:2]
    H, W = h * scale, w * scale
    ...
    for i in range(0, len(tiles), batch):
        ...
        with torch.no_grad():
            pred = model.sample(lr_b, msi_b)
            for j, (y0, y1) in enumerate(coords):
                out[:, y0:y1, y0:y1] = pred[j, :, :y1 - y0, :y1 - y0].cpu().numpy()
    return out.transpose(1, 2, 0)


def load_checkpoint(path: str, cfg: Config, device: torch.device) -> ManifoldFlow:

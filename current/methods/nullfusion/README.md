...
(`daetf.ambiguity.operator.CombinedOperator`, verified in `selfcheck`):
...
`krylovnet/notebooks/krylovnet_SOTA_CAVE_Nikon.ipynb`, with `KrylovNet` swapped
for `NullFusionNet`.  The notebook is provided at
`nullfusion/notebooks/nullfusion_SOTA_CAVE_Nikon.ipynb`.

## Run
```powershell
$env:PYTHONPATH="common;methods"
python -c "import nullfusion.nullfusion as m; m.selfcheck.run_all()"   # unit checks
python methods/nullfusion/nullfusion\train_sota.py --smoke                     # training-path self-test
```

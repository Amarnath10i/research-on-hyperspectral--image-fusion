"""
SOTA-Beating HSI-MSI Fusion for Chikusei (128 bands)
=====================================================

Target: Beat CoFusion 49.14 / RAMoE 48.10 / SMGU-Net 48.82 PSNR

Architecture: NullFusion-Progressive with:
  1. Multi-scale U-Net encoder with skip connections
  2. Spectral-Spatial Transformer blocks
  3. Multi-resolution spectral dictionary (3 scales)
  4. 3-level wavelet high-frequency refinement
  5. Channel attention + spatial attention
  6. Dense skip connections
  7. Exact null-space projection for consistency
  8. Progressive training (small -> large patches)
  9. Multi-loss: L1 + SSIM + SAM + physics + consistency
  10. EMA + gradient accumulation + warmup + cosine restarts

Usage:
  python train_chikusei_sota.py --root /kaggle/input/chikusei --output_dir /kaggle/working
"""
...
# ════════════════════════════════════════════════════════════════════════════
# NullFusion-SOTA — The SOTA-Beating Model
# ════════════════════════════════════════════════════════════════════════════

class NullFusionSOTA(nn.Module):
    """
    Multi-scale U-Net encoder + spectral dictionary prior + wavelet refinement.
    Designed to beat CoFusion 49.14 on Chikusei x4.
    """
    def __init__(self, bands=128, msi=3, width=96, scale=4,
                 dict_g=96, dict_m=64, dict_f=48):
        super().__init__()
        self.bands = bands
        W = width
        ...
        self.cross1 = CrossAttn(W, 8)
        self.cross2 = CrossAttn(W, 8)
        self.cross3 = CrossAttn(W, 8)

        # --- U-Net Encoder (3 levels) ---
        self.fuse_in = nn.Conv2d(W*2, W, 1)  # fuse HSI+MSI features
        self.enc1 = nn.Sequential(*[RCAB(W) for _ in range(3)])
        self.down1 = nn.Conv2d(W, W*2, 3, 2, 1)
        self.enc2 = nn.Sequential(*[RCAB(W*2) for _ in range(3)])
        self.down2 = nn.Conv2d(W*2, W*4, 3, 2, 1)
        self.bottleneck = nn.Sequential(*[RCAB(W*4) for _ in range(4)])

        # --- U-Net Decoder with skip connections ---
        self.up2 = nn.ConvTranspose2d(W*4, W*2, 2, 2)
        self.dec2 = nn.Sequential(*[RCAB(W*2) for _ in range(3)])
        self.up1 = nn.ConvTranspose2d(W*2, W, 2, 2)
        self.dec1 = nn.Sequential(*[RCAB(W) for _ in range(3)])

        # --- Spatial attention in decoder ---
        ...
        for i in range(10):
            ...
            if i % 3 == 2: prior_blocks.append(SpectralMix(W))
        prior_blocks.extend([nn.Conv2d(W, bands, 3,1,1)])
        self.prior = nn.Sequential(*prior_blocks)
        # Project prior output back to W for code heads
        ...
        # --- Wavelet branch ---
        self.wavelet = WaveletBranch(bands, W//2, depth=3)
    ...
    def forward(self, yH, yM):
        ...
        v = self.prior_proj(v_raw)  # bands -> W
        ...
        sw = F.softmax(self.scale_w(d1_hr), -1).unsqueeze(-1).unsqueeze(-1)  # (B,3,1,1)
...
class ChikuseiDS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            if random.random() < 0.15:  # noise augmentation
...
def train(args):
    ...
    if torch.cuda.is_available():
        ...
        # Adaptive settings based on GPU memory
        if mem < 12:
            patch_size, batch_size, width = 48, 1, 80
            print("[Config] Small GPU: patch=48, batch=1, width=80")
        elif mem < 16:
            patch_size, batch_size, width = 64, 2, 96
            print("[Config] Medium GPU: patch=64, batch=2, width=96")
        else:
            patch_size, batch_size, width = 80, 2, 112
            print("[Config] Large GPU: patch=80, batch=2, width=112")
    else:
        patch_size, batch_size, width = 64, 1, 64
        print("[Config] CPU: patch=64, batch=1, width=64")

    if args.patch_size: patch_size = args.patch_size
    if args.batch_size: batch_size = args.batch_size

    # Model
    model = NullFusionSOTA(bands=128, msi=3, width=width, scale=4,
                           dict_g=96, dict_m=64, dict_f=48).to(device)
    nparams = sum(p.numel() for p in model.parameters())
    print(f"\nModel: NullFusionSOTA — {nparams/1e6:.2f}M params")
    ...
    best_psnr = 0
    best_epoch = 0
    save_dir = os.path.join(args.output_dir, "nullfusion_sota_chikusei")
    ...
    for epoch in range(1, epochs+1):
        ...
        for step in range(steps):
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                loss_consist = F.l1_loss(out["consist"], gt)
                loss_null = out["null"].abs().mean() * 0.01

                loss = loss_l1 + 0.1*loss_phys + 0.05*loss_consist + loss_null
        ...
        if epoch % args.eval_every == 0 or epoch == epochs:
            ...
            with torch.no_grad():
                ...
                for i in range(n_eval):
                    ...
                    ss.append(ssim(pred_np, gt_np))
                    sa.append(sam(pred_np, gt_np))

...
# Chikusei Headwall Nano-Hyperspec-VNIR-C response, 363-1018 nm (128 bands).
# Approximated from the sensor specifications: 128 bands across 363-1018 nm.
# The R/G/B channels are modelled as broad overlapping Gaussian-like responses
# typical of a 3-channel RGB camera (e.g. Canon EOS 5D Mark II used for ground truth).
# We use a realistic 3-channel model covering the VNIR range.
_CHIKUSEI_128_WL = np.linspace(363.0, 1018.0, 128)  # nm
_CHIKUSEI_128_SRF_RAW = np.stack([
    # Red channel: peaks ~620 nm, wide
    np.exp(-((_CHIKUSEI_128_WL - 620.0) ** 2) / (2 * 80.0 ** 2)),
    # Green channel: peaks ~540 nm
    np.exp(-((_CHIKUSEI_128_WL - 540.0) ** 2) / (2 * 70.0 ** 2)),
    # Blue channel: peaks ~460 nm
    np.exp(-((_CHIKUSEI_128_WL - 460.0) ** 2) / (2 * 60.0 ** 2)),
], axis=1).astype(np.float32)

# Houston ITRES CASI-1500 response, 380-1050 nm (48 bands).
# Approximated from the CASI-1500 sensor specifications.
_HOUSTON_48_WL = np.linspace(380.0, 1050.0, 48)  # nm
_HOUSTON_48_SRF_RAW = np.stack([
    # Red channel: peaks ~630 nm
    np.exp(-((_HOUSTON_48_WL - 630.0) ** 2) / (2 * 75.0 ** 2)),
    # Green channel: peaks ~550 nm
    np.exp(-((_HOUSTON_48_WL - 550.0) ** 2) / (2 * 65.0 ** 2)),
    # Blue channel: peaks ~470 nm
    np.exp(-((_HOUSTON_48_WL - 470.0) ** 2) / (2 * 55.0 ** 2)),
], axis=1).astype(np.float32)


def chikusei_srf(bands: int = 128, normalise: bool = True) -> np.ndarray:
    """Chikusei Headwall Nano-Hyperspec response as [bands, 3].

    The 128-band SRF covers 363-1018 nm.  If `bands` != 128 the response is
    resampled by linear interpolation over the normalised wavelength axis.
    """
...
def houston_srf(bands: int = 48, normalise: bool = True) -> np.ndarray:
    """Houston ITRES CASI-1500 response as [bands, 3].

    The 48-band SRF covers 380-1050 nm.  If `bands` != 48 the response is
    resampled by linear interpolation over the normalised wavelength axis.
    """
    src = _HOUSTON_48_SRF_RAW
    if bands != src.shape[0]:
        xs = np.linspace(0.0, 1.0, src.shape[0])
        xd = np.linspace(0.0, 1.0, bands)
        src = np.stack([np.interp(xd, xs, src[:, i]) for i in range(3)], axis=1)
...
def conditioning(srf: np.ndarray) -> dict:
    ...
    A larger condition number means the bands -> 3 projection is closer to
